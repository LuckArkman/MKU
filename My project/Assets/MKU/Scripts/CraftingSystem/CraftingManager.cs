...
namespace MKU.Scripts.CraftingSystem
{
    ...
    public class CraftingManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            Singleton.Instance._craftingManager = this;
        }
        ...
        public async Task<bool> Craft(Recipe recipe)
        {
            ...
            if (Singleton.Instance._character == null) response = await new FinanceManager().PostCsts(new Message(Singleton.Instance.Id, ActionCode.Transference, recipe.price, Singleton.Instance._craftingManager.Id));
            if (Singleton.Instance._character != null) response = await new FinanceManager().PostCsts(new Message(Singleton.Instance._character.id, ActionCode.Transference, recipe.price, Singleton.Instance._craftingManager.Id));
            ...
            if (response == "200")
            {
                ...
                var sucess = new Random().NextDouble() <= (recipe.parcent / 100);
                if (sucess) inventory.AddToFirstEmptySlot(_container.items.Find(x => x.itemID == recipe.result), 1);
                Debug.Log($"Crafted:  >> {sucess} >> {recipe.result}");
                return true;
            }
            return false;
        }
